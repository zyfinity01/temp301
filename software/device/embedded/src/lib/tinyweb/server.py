...
async def restful_resource_handler(req, resp, param=None):
    ...
    log.debug(
        "handler: {0}, Kwargs: {1}, param: {2}, data: {3}".format(
            _handler, _kwargs, param, data
        )
    )

    # Slight modification to rest API to handle async coroutines.
    # async methods within resource class must be decorated with @staticmethod,
    # so that the handler is able to tell the method is coroutine (otherwise it sees only bound_method)
    # IMPORTANT!!!!! THIS HOTFIX BREAKS GENERATOR RESPONSES.
    # Because uPy doesn't differentiate between coroutines and generators, the if statement sees them as the same thing.
    # If the _handler is a generator, It will try await it, rather than draining the generator like the code further down tries to do.
    # see mr !152
    if isinstance(_handler, asyncio.type_genf):
        log.debug("Handling REST API request with coroutine")
        if param:
            res = await _handler(data, param, **_kwargs)
        else:
            res = await _handler(data, **_kwargs)
    else:
        # handle normally
        if param:
...
class webserver:
    ...
    async def _handler(self, reader, writer):
        ...
        try:
            ...
            if self.debug:
                log.debug(
                    "reader: {0} {1} {2} {3} {4}".format(
                        req.reader, req.headers, req.method, req.path, req.query_string
                    )
                )
